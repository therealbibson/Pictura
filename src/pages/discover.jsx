...
const Gallery = () => {
  ...
  const handleSave = (e, item) => {
    ...
    const isAlreadySaved = saved.some(s => s.src === item.img);

    if (isAlreadySaved) {
      saved = saved.filter(s => s.src !== item.img);
    } else {
      saved = [...saved, { id: Date.now() + Math.random(), src: item.img }];
    }
        ...
        {galleryItems.map((item, index) => (
          ...
          >
            ...
            <div className="absolute top-2 right-2 z-10 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition">
              ...
              >
                {savedItems.some(s => s.src === item.img) ? <FaBookmark className="text-[#ff8c42]" /> : <FaRegBookmark />}
              </button>
            </div>
          </div>
        ))}
